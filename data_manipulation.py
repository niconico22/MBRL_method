...
def from_observation_to_usablestate(states, which_agent, just_one):
    ...
    if(which_agent == 1):
        if(just_one):
            ...
            # 9 vals of rot mat --> 6 vals (cos sin of rpy)
            body_rpy = to_euler(curr_item[29:38], just_one)
            body_vel = curr_item[41:44]
            full_item = np.concatenate(
                (joint_pos, joint_vel, body_pos, body_rpy, body_vel), axis=0)
            return full_item
            ...
            for i in range(len(states)):  # for each rollout
                ...
                full_item = np.concatenate(
                    (joint_pos, joint_vel, body_pos, body_rpy, body_vel), axis=1)
...
def to_euler(rot_mat, just_one):
    if(just_one):
        ...
        p = np.arctan2(-rot_mat[6], np.sqrt(rot_mat[7]
                                            * rot_mat[7]+rot_mat[8]*rot_mat[8]))
        ...
        r = np.arctan2(rot_mat[:, 3], rot_mat[:, 1])
        r = np.concatenate((np.expand_dims(np.cos(r), axis=1),
                            np.expand_dims(np.sin(r), axis=1)), axis=1)

        p = np.arctan2(-rot_mat[:, 6], np.sqrt(rot_mat[:, 7]
                                               * rot_mat[:, 7]+rot_mat[:, 8]*rot_mat[:, 8]))
        p = np.concatenate((np.expand_dims(np.cos(p), axis=1),
                            np.expand_dims(np.sin(p), axis=1)), axis=1)

        y = np.arctan2(rot_mat[:, 7], rot_mat[:, 8])
        y = np.concatenate((np.expand_dims(np.cos(y), axis=1),
                            np.expand_dims(np.sin(y), axis=1)), axis=1)
