...
class Buffer:
    def __init__(self, d_state, d_action, d_reward, ensemble_size, buffer_size):
        """
        ...
        self.rewards = torch.zeros(buffer_size, d_reward).float()
    ...
    def add(self, state, action, next_state, reward):
        ...
        """
        state, action, next_state = torch.from_numpy(state).float().clone(), torch.from_numpy(
            action).float().clone(), torch.from_numpy(next_state).float().clone()
    ...
    def train_batches(self, batch_size):
        ...
        for i in range(0, num, batch_size):
            ...
            states = states.reshape(
                self.ensemble_size, batch_size, self.d_state)
            actions = actions.reshape(
                self.ensemble_size, batch_size, self.d_action)
            state_deltas = state_deltas.reshape(
                self.ensemble_size, batch_size, self.d_state)
            rewards = rewards.reshape(
                self.ensemble_size, batch_size, self.d_reward)
            yield states, actions, state_deltas, rewards
    ...
    def pick(self):
        ...
        next_state = self.states[idx] + self.state_deltas[idx]
        return action, state, next_state
