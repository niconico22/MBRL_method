...
######################################################################################################################
    def ant_follow_traj(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        scores[min_perp_dist < 1] += (min_perp_dist *
                                      self.horiz_penalty_factor)[min_perp_dist < 1]
        scores[min_perp_dist >= 1] += (min_perp_dist*10 *
                                       self.horiz_penalty_factor)[min_perp_dist >= 1]

        # encourage moving-forward
        scores[moved_to_next == 0] -= self.forward_encouragement_factor * \
            (curr_forward - prev_forward)[moved_to_next == 0]
        scores[moved_to_next == 1] -= self.forward_encouragement_factor * \
            (curr_forward)[moved_to_next == 1]

        # prevent height from going too high or too low
        scores[pt[:, self.z_index] > 0.67] += (
            self.heading_penalty_factor*40 + 0*pt[:, self.z_index])[pt[:, self.z_index] > 0.67]
        scores[pt[:, self.z_index] < 0.3] += (
            self.heading_penalty_factor*40 + 0*pt[:, self.z_index])[pt[:, self.z_index] < 0.3]
    ...
    def swimmer_follow_traj(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        angle = np.arctan2(
            curr_line_end[:, 1]-curr_line_start[:, 1], curr_line_end[:, 0]-curr_line_start[:, 0])
        # ^ -pi to pi
        ...
        my_range = np.pi/3.0
        scores[diff <
               my_range] += (self.heading_penalty_factor*diff)[diff < my_range]
...
######################################################################################################################
    def ant_forward(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        if(pt_number == all_samples.shape[1]):
            scores[done_forever == 0] += 0.005*np.sum(
                np.square(all_samples[:, pt_number-1, :][done_forever == 0]/scaling), axis=1)
        else:
            scores[done_forever == 0] += 0.005*np.sum(
                np.square(all_samples[:, pt_number, :][done_forever == 0]/scaling), axis=1)
    ...
    def swimmer_forward(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        if(pt_number == all_samples.shape[1]):
            reward_ctrl = 0.5 * \
                np.sum(
                    np.square(all_samples[:, pt_number-1, :]/scaling), axis=1)
        else:
            reward_ctrl = 0.5 * \
                np.sum(np.square(all_samples[:, pt_number, :]/scaling), axis=1)
    ...
    def cheetah_forward(self, pt, prev_pt, scores, min_perp_dist, curr_forward, prev_forward,
        ...
        if(pt_number == all_samples.shape[1]):
            scores += 0.05 * \
                np.sum(np.square(all_samples[:, pt_number-1, :]), axis=1)
        else:
            scores += 0.05 * \
                np.sum(np.square(all_samples[:, pt_number, :]), axis=1)
