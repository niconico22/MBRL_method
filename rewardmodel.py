...
class RewardModel(nn.Module):
        ...
        for lyr_idx in range(n_layers + 1):
            if lyr_idx == 0:
                lyr = EnsembleDenseLayer(
                    d_action + d_state, n_hidden, ensemble_size, non_linearity=non_linearity)
            elif 0 < lyr_idx < n_layers:
                lyr = EnsembleDenseLayer(
                    n_hidden, n_hidden, ensemble_size, non_linearity=non_linearity)
            elif lyr_idx == n_layers:
                lyr = EnsembleDenseLayer(
                    n_hidden, d_reward + d_reward, ensemble_size, non_linearity='linear')
            layers.append(lyr)
    ...
    def _pre_process_model_targets(self, rewards):
        rewards = rewards.to(self.device)

        if self.normalizer is None:
            return rewards

        rewards = self.normalizer.normalize_state_deltas(rewards)
        return rewards

    def _post_process_model_outputs(self, reward_mean, var):
        # denormalize to return in raw state space
        if self.normalizer is not None:
            reward_mean = self.normalizer.denormalize_state_delta_means(
                reward_mean)
            var = self.normalizer.denormalize_state_delta_vars(var)
        return reward_mean, var

    def _propagate_network(self, states, actions):
        inp = torch.cat((states, actions), dim=2)
        op = self.layers(inp)
        reward_mean, log_var = torch.split(op, op.size(2) // 2, dim=2)
    ...
    def forward(self, states, actions):
        ...
        normalized_states, normalized_actions = self._pre_process_model_inputs(
            states, actions)
        normalized_reward_mean, normalized_var = self._propagate_network(
            normalized_states, normalized_actions)
        reward_mean, var = self._post_process_model_outputs(
            normalized_reward_mean, normalized_var)
        #next_state_mean = delta_mean + states.to(self.device)
        reward_mean = reward_mean.to(self.device)
    ...
    def forward_all(self, states, actions):
        ...
        return reward_means.transpose(0, 1), reward_vars.transpose(0, 1)
    ...
    def loss(self, states, actions, rewards, training_noise_stdev=0):
        """
        ...
        targets = self._pre_process_model_targets(rewards)
        ...
        # mu, var = self._propagate_network(
        #    states, actions)      # delta and variance
        mu, var = self.forward(states, actions)      # delta and variance
    ...
    def likelihood(self, states, actions, rewards):
        """
        ...
        log_likelihood = pdf.log_prob(rewards)
